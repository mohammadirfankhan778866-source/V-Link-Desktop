...
fun decodeBase64(input: String): ByteArray {
  ...
  return decoder.javaClass.getMethod("decode", String::class.java).invoke(decoder, input) as ByteArray
}
...
android {
  ...
  defaultConfig {
    applicationId = "com.aistudio.pulsechat.jdowtf"
    minSdk = 24
    targetSdk = 35
    versionCode = 1
    versionName = "1.0"
  ...
  signingConfigs {
    create("release") {
      ...
      if (keystoreFile.exists() && !storePasswordEnv.isNullOrBlank()) {
        ...
        // Fallback to debug config so that builds succeed on CI/CD when keys are not configured yet
        val debugKeystoreFile = file("${rootDir}/debug.keystore")
        if (!debugKeystoreFile.exists()) {
          val base64File = file("${rootDir}/debug.keystore.base64")
          if (base64File.exists()) {
            val decoded = decodeBase64(base64File.readText().trim())
